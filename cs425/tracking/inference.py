...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
        noisyDistance = observation                                         # observed values
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            self.particles = [self.getJailPosition() for i in range(0, self.numParticles)]
        else:
            for pos in self.legalPositions:
                distance = util.manhattanDistance(pos, pacmanPosition)
                allPossible[pos] += emissionModel[distance] * beliefs[pos]
